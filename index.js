...
const anthropic = new Anthropic({ apiKey: 'sk-ant-api03-7_CU85qhqKJnshbDHJMQVfPC291djiEQep8UY0_kV3PDn4JXtESW-WMoUyOY1ScQj097379fXGYcZpoUR2wCNg-BLAjgQAA' });
const explorers = { ethereum: 'https://api.etherscan.io/api', bsc: 'https://api.bscscan.com/api', polygon: 'https://api.polygonscan.com/api', arbitrum: 'https://api.arbiscan.io/api' };
app.post('/analizar', async (req, res) => {
  try {
    ...
    if (verificado && data.SourceCode) {
      const msg = await anthropic.messages.create({ model: 'claude-opus-4-6', max_tokens: 1024, messages: [{ role: 'user', content: 'Analiza este smart contract en espanol. Di que hace, riesgos, nivel BAJO/MEDIO/ALTO. Codigo: ' + data.SourceCode.substring(0, 3000) }] });
